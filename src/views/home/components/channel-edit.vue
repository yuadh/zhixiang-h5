<template>
  <div class="channel-edit">
    <van-cell :border="false">
      <div slot="title" class="title-text">我的板块 <span>点击进入板块</span></div>
      <van-button
        class="edit-btn"
        type="danger"
        plain
    ...
    <!-- 频道推荐 -->
    <van-cell :border="false">
      <div slot="title" class="title-text">更多板块<span>点击添加板块</span></div>
    </van-cell>
  ...
  methods: {
    ...
    async onAddChannel (channel) {
      ...
      if (this.user) {
        ...
        // 未登录，把数据存储到本地
        setItem(ZHIXIANG_CHANNEL, this.myChannels)
      }
    },
  ...
  /deep/ .recommend-grid {
    .grid-item {
      .van-grid-item__content {
        ...
        repeating-linear-gradient(-45deg,#ccc 0, #ccc 0.2em,white 0,white 0.4em);
        border-radius: 10%;
        .van-icon-add {
          position: absolute;
          right: -10px;
